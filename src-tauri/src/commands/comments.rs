...
pub fn list_comments(state: State<AppState>, issue_id: i64) -> Result<Vec<Comment>, String> {
    state.rt.block_on(async {
        sqlx::query_as::<_, Comment>("SELECT * FROM comments WHERE issue_id = ? ORDER BY created_at ASC")
            .bind(issue_id)
...
pub fn create_comment(state: State<AppState>, input: CreateCommentInput) -> Result<Comment, String> {
    ...
    state.rt.block_on(async {
        ...
        sqlx::query("UPDATE issues SET updated_at = ? WHERE id = ?")
            .bind(&now)
            .bind(input.issue_id)
            .execute(&state.pool)
            .await?;
...
pub fn comment_count(state: State<AppState>, issue_id: i64) -> Result<i64, String> {
    state.rt.block_on(async {
        sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM comments WHERE issue_id = ?")
            .bind(issue_id)
            .fetch_one(&state.pool)
            .await
    }).map_err(|e| e.to_string())
