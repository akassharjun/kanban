...
pub fn list_templates(state: State<AppState>, project_id: i64) -> Result<Vec<IssueTemplate>, String> {
    state.rt.block_on(async {
        sqlx::query_as::<_, IssueTemplate>("SELECT * FROM issue_templates WHERE project_id = $1 ORDER BY name")
...
pub fn create_template(state: State<AppState>, input: CreateTemplateInput) -> Result<IssueTemplate, String> {
    state.rt.block_on(async {
        ...
        let id: i64 = sqlx::query_scalar(
            "INSERT INTO issue_templates (project_id, name, description_template, default_status_id, default_priority, default_label_ids, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id"
        )
        ...
        sqlx::query_as::<_, IssueTemplate>("SELECT * FROM issue_templates WHERE id = $1")
...
pub fn update_template(state: State<AppState>, id: i64, input: UpdateTemplateInput) -> Result<IssueTemplate, String> {
    state.rt.block_on(async {
        ...
        if let Some(ref name) = input.name {
            sqlx::query("UPDATE issue_templates SET name = $1, updated_at = $2 WHERE id = $3")
                .bind(name).bind(&now).bind(id).execute(&state.pool).await?;
        }
        if let Some(ref desc) = input.description_template {
            sqlx::query("UPDATE issue_templates SET description_template = $1, updated_at = $2 WHERE id = $3")
                .bind(desc).bind(&now).bind(id).execute(&state.pool).await?;
        }
        if let Some(status_id) = input.default_status_id {
            sqlx::query("UPDATE issue_templates SET default_status_id = $1, updated_at = $2 WHERE id = $3")
                .bind(status_id).bind(&now).bind(id).execute(&state.pool).await?;
        }
        if let Some(ref priority) = input.default_priority {
            sqlx::query("UPDATE issue_templates SET default_priority = $1, updated_at = $2 WHERE id = $3")
        ...
        if let Some(ref label_ids) = input.default_label_ids {
            ...
            sqlx::query("UPDATE issue_templates SET default_label_ids = $1, updated_at = $2 WHERE id = $3")
...
pub fn delete_template(state: State<AppState>, id: i64) -> Result<(), String> {
    state.rt.block_on(async {
        sqlx::query("DELETE FROM issue_templates WHERE id = $1").bind(id).execute(&state.pool).await?;
        Ok(())
    }).map_err(|e: sqlx::Error| e.to_string())
}
