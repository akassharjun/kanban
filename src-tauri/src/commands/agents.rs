...
pub fn register_agent(state: State<AppState>, input: RegisterAgentInput) -> Result<Agent, String> {
    state.rt.block_on(async {
        ...
        sqlx::query(
            "INSERT INTO agents (id, name, type, skills, task_types, max_concurrent, max_complexity, status, registered_at, last_heartbeat) VALUES (?, ?, ?, ?, ?, ?, ?, 'idle', ?, ?)"
        )
        .bind(&id)
        .bind(&input.name)
        .bind(&input.r#type)
...
pub fn agent_heartbeat(state: State<AppState>, agent_id: String) -> Result<Agent, String> {
    state.rt.block_on(async {
        ...
        let updated = sqlx::query_as::<_, Agent>("SELECT * FROM agents WHERE id = ?")
            .bind(&agent_id)
            .fetch_one(&state.pool)
            .await?;
...
pub fn deregister_agent(state: State<AppState>, agent_id: String) -> Result<(), String> {
    state.rt.block_on(async {
        ...
        for (issue_id,) in &active_tasks {
            sqlx::query(
                "UPDATE task_contracts SET task_state = 'queued', claimed_by = NULL, claimed_at = NULL WHERE issue_id = ? AND claimed_by = ?"
            )
            .bind(issue_id)
            .bind(&agent_id)
            .execute(&state.pool)
            ...
            sqlx::query(
                "INSERT INTO execution_logs (issue_id, agent_id, attempt_number, entry_type, message, timestamp) VALUES (?, ?, 0, 'reclaimed', 'Agent deregistered, task reclaimed to queue', ?)"
            )
            .bind(issue_id)
            .bind(&agent_id)
            .bind(&now)
