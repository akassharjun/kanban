...
pub async fn invalidate_task(pool: &SqlitePool, issue_id: i64, reason: &str) -> Result<CascadeResult, sqlx::Error> {
    ...
    // Log invalidation
    sqlx::query(
        "INSERT INTO execution_logs (issue_id, agent_id, attempt_number, entry_type, message, timestamp) VALUES (?, 'system', ?, 'error', ?, ?)"
    ...
    for downstream_id in &all_downstream {
        ...
        match ds_contract.task_state.as_str() {
            "queued" | "claimed" | "blocked" => {
                // Block these tasks
                sqlx::query("UPDATE task_contracts SET task_state = 'blocked', claimed_by = NULL, claimed_at = NULL WHERE issue_id = ?")
                ...
                sqlx::query(
                    "INSERT INTO execution_logs (issue_id, agent_id, attempt_number, entry_type, message, timestamp) VALUES (?, 'system', 1, 'error', 'Blocked due to upstream invalidation', ?)"
            ...
            "executing" => {
                ...
                sqlx::query(
                    "INSERT INTO execution_logs (issue_id, agent_id, attempt_number, entry_type, message, timestamp) VALUES (?, 'system', ?, 'error', 'WARNING: Upstream task invalidated, review work on completion', ?)"
            ...
            "completed" => {
                ...
                let review_result = sqlx::query(
                    "INSERT INTO issues (project_id, identifier, title, description, status_id, priority, position, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 0.0, ?, ?)"
                ...
                let review_id = review_result.last_insert_rowid();

                sqlx::query(
                    "INSERT INTO task_contracts (issue_id, type, task_state, objective, required_skills, estimated_complexity, timeout_minutes) VALUES (?, 'review', 'queued', ?, '[\"review\"]', 'small', 30)"
                ).bind(review_id)
    ...
    sqlx::query(
        "INSERT INTO notifications (type, issue_id, message, read, created_at) VALUES ('cascade_failure', ?, ?, 0, ?)"
    ).bind(issue_id)
