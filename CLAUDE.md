...
- **Semantic tools**, not a generic apply: reads (`list_projects`, `list_statuses`, `list_labels`, `list_members`, `list_issues`, `get_issue`, `search_issues`) + writes (`create_project`, `create_issue`, `update_issue`, `move_issue`, `create_member`, `update_member`, `delete_member`, `assign_issue`, `undo`, `redo`). Tools resolve human identifiers — project `prefix`, issue `key`, status/label/member `name` — to core UUIDs; misses map to `not_found`/`unknown_name` MCP errors. Output DTOs (`convert.rs`) are human-facing (no UUIDs/sort_keys).
...
- **Member tools (Spec #7).** `list_members`/`create_member`/`update_member`/`delete_member` (members referenced by name within a project) + `assign_issue { key, member? }` (omit `member` to unassign). Assignment reuses `Operation::UpdateIssueField` with `IssueFieldChange::Assignee` — no new core op. `IssueOut` carries `assignee: Option<String>` (the member name, or `null`), resolved via a `member_name_map` threaded through every issue-returning tool; `search_issues` collects members across all projects like it already does statuses. Pure MCP-surface + output-shape addition — no schema/core change. See `crates/kanban-mcp/tests/members_flow.rs`.
- **Agent orchestration (registry, task contracts, decomposition, validation) is deferred to Spec #8+.**
...
- `docs/superpowers/specs/2026-06-06-kanban-v2-spec-7-mcp-member-tools-design.md` — Spec #7 (MCP member tools).

Each spec has a matching plan in `docs/superpowers/plans/`. AI-agent orchestration and other follow-ups (Spec #8+) will live alongside.
